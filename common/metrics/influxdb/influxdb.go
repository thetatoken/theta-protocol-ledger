...
func InfluxDBWithTags(r metrics.Registry, d time.Duration, url, database, username, password, namespace string, tags map[string]string) {
	...
	if err != nil {
		log.Warnf("Unable to parse InfluxDB, url: %v, err: %v", url, err)
		return
	}
	...
	for {
		...
		case <-intervalTicker:
			if err := r.send(); err != nil {
				log.Warnf("Unable to send to InfluxDB, err: %v", err)
			}
		case <-pingTicker:
			_, _, err := r.client.Ping()
			if err != nil {
				log.Warnf("Got error while sending a ping to InfluxDB, trying to recreate client, err: %v", err)

				if err = r.makeClient(); err != nil {
					log.Warnf("Unable to make InfluxDB client, err: %v", err)
				}
			}
		}
	}
